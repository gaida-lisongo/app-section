...
import SemestreService, { Semestre, SemestreWithUnites } from "@/app/services/SemestreService";

const CycleSemestre = ({semestreId} : {semestreId: string}) => {
  const [ semestre, setSemestre] = React.useState<SemestreWithUnites | null>(null);

  useEffect(() => {
    const fetchSemestre = async () => {
      const semestreData = await SemestreService.getSemestre(semestreId);
      console.log("Semestre:", semestreData);
