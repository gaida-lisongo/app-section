...
const Footer = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    router.push(`/payment?matricule=${encodeURIComponent(data.matricule.trim())}`);
  };

  const onClose = (payment: boolean) => {
    if (payment) {
      router.push(`/resultat?matricule=${encodeURIComponent(data.matricule.trim())}`);
    }
    setIsOpen(false);
  };
